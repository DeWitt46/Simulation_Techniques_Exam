...
int main() { // Begin main program without card file.
	...
	TFile* outFile = new TFile("PbPb5TeV.root", "RECREATE");
	
	// Book histogram.
	TH1F *mult = new TH1F("mult","charged multiplicity", 400, -0.5, 13000.5);
	...
	cout << "Pb-Pb at 5.02 TeV" << endl;		
	cout << "!ccccccccccccccccccccccccccccccc" << endl;
	cout << endl;
			
	// Read in commands from external file doesn't work with parallel.
	pythia.readFile("exam_settings.cmnd", 2);
	...
	Pythia reader;
	reader.readFile("exam_settings.cmnd", 2);	
