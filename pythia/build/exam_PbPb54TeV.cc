...
// To include this to get access to the HIInfo object for HeavyIons.
...
int main() { // Begin main program without card file.
	...
	TFile* outFile = new TFile("PbPb54TeV.root", "RECREATE");
	
	// Book histogram.
	TH1F *mult = new TH1F("mult","charged multiplicity", 400, -0.5, 15000.5);
	TH1F *p_pT = new TH1F("p_pT","proton pT", 400, -0.1, 5);
	TH1F *pi_pT = new TH1F("pi_pT","pion pT", 400, -0.1, 5);

	cout << endl;
	cout << "!ccccccccccccccccccccccccccccccc" << endl;
	cout << "Pb-Pb at 5.4 TeV" << endl;		
	cout << "!ccccccccccccccccccccccccccccccc" << endl;
	cout << endl;
			
	// Read in commands from external file doesn't work with parallel.
	pythia.readFile("exam_settings.cmnd", 3);
	...
	Pythia reader;
	reader.readFile("exam_settings.cmnd", 3);	
